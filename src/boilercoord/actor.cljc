(ns boilercoord.actor
  "BoilerCoordActor — the ISCO-08 8182 Steam Engine and Boiler
  Operators boiler-plant scheduling/logistics coordination actor as a
  ...
  resume after human sign-off. This actor coordinates BOILER-PLANT
  SCHEDULING/LOGISTICS ONLY — it never operates boiler/steam equipment
  itself, never makes a boiler-operation-execution or plant-safety-
  clearance decision, and never commits a proposal the
  BoilerCoordGovernor refuses. Modeled on cloud-itonami-isco-8131's
  chemcoord.actor.
  ...
  The unconditional invariant: the Boiler-Plant Scheduling &
  Logistics Coordination Advisor can never directly commit a
  coordination record the BoilerCoordGovernor refuses — every
  commit-record! call is gated behind `:decide`, and the governor's
  scope-exclusion rule makes any proposal to finalize a boiler-
  ...
  or override a plant safety officer's judgment an unconditional,
  permanent :hold."
...
(defn build-graph
  "Build a compiled BoilerCoordActor graph. `store` implements
  `boilercoord.store/Store`. `advisor` implements
  `boilercoord.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:plant-id (:plant-id request)
                                    :op (:op proposal)
                                    :operator-id (:operator-id proposal)
                                    :payload proposal}]
...
(defn approve!
  ...
  can never be used to approve a boiler-operation-execution-decision
  finalization, a plant-safety-clearance-decision finalization, or a
  plant-safety-officer's-judgment override."
  [graph thread-id]
